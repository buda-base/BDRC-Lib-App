...
class VolumeCard extends Component {
	...
	// https://www.tbrc.org/browser/PdfServiceApp/?endPage=4&startPage=1&work=W00KG01653&volume=I00KG02965 
	onPDFButtonClicked = () => {
...
class PDFDownload extends Component {
	...
	queryForPDF = () => {
		...
		let generationURL = 'https://www.tbrc.org/browser/PdfServiceApp/?endPage='+this.props.endPage+'&startPage='+startPage+'&work='+this.props.workId+'&volume='+this.props.volumeId;
		...
		if(null==json) {
			...
			if(false==json.success) {
				...
				if('done'===json.status) {	
					...
					this.pdfPageLink = 'https://www.tbrc.org/'+('cn'===this.props.strings.id?'locale=zh':'')+'#pdfdl?endPage='+this.props.endPage+'&startPage='+startPage+'&work='+this.props.workId+'&volume='+this.props.volumeId;
			...
			<Dialog isOpen={this.visible} onPostHide={this.handlePostHide}>
				<div className="PDFDownload">
					...
					{this.pdfHasBeenGenerated 
						...
						:
	  				this.props.strings.pleaseWaitWhileWeGenerateYourPDF
	  			}
	  			</p>
	  			<div>
	  			{this.pdfHasBeenGenerated ?
	  				...
	  				:
	  				(this.errorMode ?
	  					<span>{this.props.strings.anErrorOccurred}</span>
	  					:
...
class OutlineDetail extends Component {
	...
	render() {
		if(this.props.outline) {
			...
			// https://www.tbrc.org/#!rid=O4JW333|O4JW3334CZ90006$W22084
			let shareLink = work ? 'https://www.tbrc.org/?locale='+this.props.strings.id+'#!rid='+this.props.outline.outlineNodeId+'|'+this.props.outline.nodeId+'$'+work.nodeId : '';
			...
			return (
				<section>
			    	...
			    	{work ? (this.state.pechaViewerOpen?<PechaViewer workId={work.nodeId} imageGroups={this.state.imageGroups} onGalleryClose={this.onGalleryClose} startPhotoIndex={0} />:null):null}
						{this.state.alertOpen?<NetworkAlert show={this.state.alertOpen} onClose={this.onAlertClose} strings={this.props.strings} />:null}
					</Card>

					{work ? (this.state.imageGroups.map((imageGroup, idx)=>{ return <OutlineDetailSection part={idx} appState={this.props.appState} key={imageGroup.imageGroupId} workId={work?work.nodeId:''} imageGroup={imageGroup} strings={this.props.strings} />; })) : null }
...
class OutlineDetailSection extends Component {
	...
	// https://www.tbrc.org/browser/PdfServiceApp/?endPage=4&startPage=1&work=W00KG01653&volume=I00KG02965 
	onPDFButtonClicked = () => {
		window.ga.trackEvent('DetailPage', 'PDF', this.props.workId+'-'+this.props.imageGroup.volumeNum);
	...
	render() {
		return (
			<Card modifier="material">
  			<div><h4>{this.props.strings.VolumePre}{this.props.strings.displayNum(this.props.imageGroup.volumeNum)}{this.props.strings.VolumePost}{this.props.strings.pagesPre} {this.props.strings.displayNum(this.props.imageGroup.end - this.props.imageGroup.start)} {this.props.strings.pagesPost}</h4></div>
  			<div className="action-bar">
  				<div className="actions"> 
						<PDFButton strings={this.props.strings} handlePDFButtonClicked={this.onPDFButtonClicked} />
					</div>
